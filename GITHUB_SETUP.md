...
## v2.0.2: исправление выбора времени

Новых переменных окружения и миграций не требуется. После публикации нового образа достаточно выполнить `docker pull` и пересоздать контейнер. Существующая `/data/bot.db` совместима.

## v2.0.3: описание базовой стоимости

Версия 2.0.3 добавляет текстовое поле `base_description` в таблицу `form_pricing`. Миграция выполняется автоматически при старте контейнера; удалять `bot.db` или пересоздавать каталог `/data` не нужно.
